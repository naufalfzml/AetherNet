...
### Requirement: Autopilot events publish through DA

The system SHALL publish every autopilot-created like and comment through the DA client before persisting the event locally.

#### Scenario: DA publish succeeds

- **WHEN** the worker creates an autopilot like or comment and DA publish returns a blob ID
- **THEN** the worker persists a `social_events` row using that DA blob ID
